...
| `unemployment_compensation` | `float` | Optional | Box 1, Unemployment compensation |
| `tax_refund` | `float` | Optional | Box 2, State or local income tax refunds, credits, or offsets |
| `refund_year` | `int` | Optional | Box 3, Box 2 amount is for tax year |
| `federal_tax_withheld` | `float` | Optional | Box 4, Federal income tax withheld |
| `rtaa_payments` | `float` | Optional | Box 5, RTAA payments |
| `grants` | `float` | Optional | Box 6, Taxable grants |
| `agriculture_payments` | `float` | Optional | Box 7, Agriculture payments |
| `business_income` | `bool` | Optional | Box 8, If checked, box 2 is trade or business income |
| `market_gain` | `float` | Optional | Box 9, Market gain |
| `state_and_local` | [`List[StateAndLocalTaxWithholding]`](../../doc/models/state-and-local-tax-withholding.md) | Optional | Boxes 10-11, State and Local tax withholding |
